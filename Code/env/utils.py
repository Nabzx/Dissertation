...
def save_trajectory_plot(trajectories: Dict[str, List[Tuple[int, int]]], grid_size: int, filename: str):
    ...
    for agent_id, positions in trajectories.items():
        ...
        ax.plot(x_coords, y_coords, color=color, alpha=0.6, linewidth=2, 
                label=f'{agent_id}', zorder=1)
        ...
        if len(positions) > 0:
            ax.scatter(x_coords[0], y_coords[0], color=color, s=200, 
                      ...
                      label=f'{agent_id} Start', zorder=3)
        
        # Mark ending position
        if len(positions) > 1:
            ax.scatter(x_coords[-1], y_coords[-1], color=color, s=200,
                      marker='s', edgecolors='black', linewidths=2,
                      label=f'{agent_id} End', zorder=3)
...
def save_reward_curve(reward_history: Dict[str, List[float]], filename: str):
    ...
    for agent_id, rewards in reward_history.items():
        ...
        ax.plot(episodes, rewards, color=color, marker='o', linewidth=2,
                markersize=4, label=agent_id, alpha=0.8)
