...
from env.utils import save_grid_screenshot, save_heatmap, plot_resource_distribution, save_trajectory_plot


def run_episode(env: GridWorldEnv, agents: Dict[str, HeuristicAgent], episode_num: int, 
                save_screenshots: bool = True, save_heatmaps: bool = True) -> Dict:
    """
    ...
    if save_heatmaps:
        for agent_id, heatmap in heatmaps.items():
            ...
            save_heatmap(
                ...
                heatmap_path,
                agent_id,
                title=f"Episode {episode_num} - {agent_id}"
            )
    ...
    episode_data = {
        ...
        'heatmap_paths': {
            agent_id: f"logs/heatmaps/episode_{episode_num:04d}_{agent_id}.png"
            for agent_id in heatmaps.keys()
        } if save_heatmaps else {},
        'resource_dist_path': resource_dist_path,
        # Store heatmaps as lists for JSON serialisation
        'heatmaps': {
            agent_id: heatmap.tolist()
            for agent_id, heatmap in heatmaps.items()
        },
        # Store trajectories
        'trajectories': {
            agent_id: positions
            for agent_id, positions in agent_trajectories.items()
        },
    }
...
def run_batch_simulation(num_episodes: int = 20, grid_size: int = 15, 
                        num_resources: int = 10, max_steps: int = 200,
                        save_screenshots: bool = True, save_heatmaps: bool = True) -> List[Dict]:
    """
    ...
    for episode_num in range(num_episodes):
        ...
        print(f"Agent 0: {resources['agent_0']} resources, "
