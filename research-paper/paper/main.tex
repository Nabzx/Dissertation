...
of what is available ($0.619 \to 0.648$ efficiency across a $2.7\times$ change in resources).
...
every density tested. Generalisation across agent counts remains open: our environment
encodes agents as distinct cell values and so caps at four, and whether the effect grows with
team size (as the credit-assignment account would predict, since each agent's contribution to
the team average shrinks as $1/N$) is untested. The cooperation score is not independent of
