...
because parameter sharing partially conceals the credit-assignment failure. Fair, efficient
coordination emerges not from a cooperative objective but as a side effect of individual
self-interest.
\end{abstract}
...
across resource densities and agent counts remains open. The shared-weight concern is
addressed directly by our ablation, which shows the effect strengthens rather than
disappears without parameter sharing. The cooperation score is not independent of efficiency
and fairness, which is why we also report the free-rider fraction and contribution Gini.
We use $5$ seeds ($3$ for the $\alpha$-sweep and the ablation); the ablation's effect sizes
...
$0.02$ per depleted node per step; $5\times5$ egocentric observation window (flattened to
...
than significance thresholds alone.
