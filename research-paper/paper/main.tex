...
\paragraph{Threats to validity.} We study one environment configuration, so generalisation
across resource densities and agent counts remains open. Our main experiments use a
shared-weight learner, meaning the ``agents'' are one policy generalising over four starting
positions rather than four independently parameterised strategies; we address this with an
independent-policy ablation (Sec.~\ref{sec:results}).\footnote{Ablation results to be inserted
once the independent-policy runs complete.} The cooperation score is not independent of
efficiency and fairness, which is why we also report the free-rider fraction and contribution
Gini. We use $5$ seeds ($3$ for the sweep and ablation). Whether the effect holds under a
centralised critic is future work; we expect the credit-assignment mechanism to persist but
its magnitude to depend on these factors.
